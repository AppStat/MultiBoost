...
namespace MultiBoost {
	...
	*/
	...
	public:
		...
		*/
		float findSingleThreshold(const vpIterator& dataBegin,
			...
			InputData* pData,
			float halfTheta = 0.0,
			vector<sRates>* pMu = NULL, vector<float>* pV = NULL, vector<float>* pV2 = NULL);
	...
	template <typename T> 
	float RegressionStumpAlgorithm<T>::findSingleThreshold(const vpIterator& dataBegin,
		const vpIterator& dataEnd,
		InputData* pData,
		float halfTheta,
		vector<sRates>* pMu, vector<float>* pV, vector<float>* pV2)
	{ 
	...
	template <typename T> 
	float RegressionStumpAlgorithm<T>::findSingleThresholdWithInit
		(const vpIterator& dataBegin,const vpIterator& dataEnd,
		InputData* pData, float halfTheta, vector<sRates>* pMu, vector<float>* pV, vector<float>* pV2 )
	{ 
