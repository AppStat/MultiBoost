...
namespace MultiBoost {
	...
	*/
	...
	public:
		...
		*/
		typedef typename vector< pair<int, T> >::reverse_iterator       vpIterator;
		/**
		* Const iterator on Pair. The pair refers to <index, value>.
		*/
		typedef typename vector< pair<int, T> >::const_reverse_iterator cvpIterator;
	...
	template <typename T> 
	float StumpAlgorithmLSHTC<T>::findSingleThreshold(const vpIterator& dataBegin,
		const vpIterator& dataEnd,
		InputData* pData,
		float halfTheta,
	...
	{ 
		...
		for( int i=0 ; i < _constantHalfEdges.size(); i++ ) _halfEdges[i] = -_constantHalfEdges[i];
	...
	template <typename T> 
	void StumpAlgorithmLSHTC<T>::findMultiThresholds(const vpIterator& dataBegin,
		const vpIterator& dataEnd,
		InputData* pData, vector<float>& thresholds,
		vector<sRates>* pMu, vector<float>* pV)
	{ 
