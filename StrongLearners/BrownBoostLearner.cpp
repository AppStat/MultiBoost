...
namespace MultiBoost {
	...
	void BrownBoostLearner::getArgs(const nor_utils::Args& args)
	{
	...
	void BrownBoostLearner::run(const nor_utils::Args& args)
	{
		// load the arguments
		this->getArgs(args);
	...
	void BrownBoostLearner::classify(const nor_utils::Args& args)
	{
		BrownBoostClassifier classifier(args, _verbose);
	...
	void BrownBoostLearner::doConfusionMatrix(const nor_utils::Args& args)
	{
		BrownBoostClassifier classifier(args, _verbose);
	...
	void BrownBoostLearner::doLikelihoods(const nor_utils::Args& args)
	{
		BrownBoostClassifier classifier(args, _verbose);
	...
	void BrownBoostLearner::doPosteriors(const nor_utils::Args& args)
	{
		BrownBoostClassifier classifier(args, _verbose);
	...
	void BrownBoostLearner::doCalibratedPosteriors(const nor_utils::Args& args)
	{
		BrownBoostClassifier classifier(args, _verbose);
	...
	float BrownBoostLearner::updateWeights(InputData* pData, BaseLearner* pWeakHypothesis)
	{
	...
	int BrownBoostLearner::resumeWeakLearners(InputData* pTrainingData)
	{
	...
	void BrownBoostLearner::resumeProcess(Serialization& ss, 
	...
	void BrownBoostLearner::printOutputInfo(OutputInfo* pOutInfo, int t, 
		InputData* pTrainingData, InputData* pTestData, 
