...
pair<int, float> ExampleResults::getWinner(int rank)
{
   assert(rank >= 0);

   vector< pair<int, float> > rankedList;
...
void ExampleResults::getRankedList( vector< pair<int, float> >& rankedList ) const
{
   rankedList.resize( _votesVector.size() );

   vector<float>::const_iterator vIt;
   const vector<float>::const_iterator votesVectorEnd = _votesVector.end();
   int i;
   for (vIt = _votesVector.begin(), i = 0; vIt != votesVectorEnd; ++vIt, ++i )
      rankedList[i] = make_pair(i, *vIt);

   sort( rankedList.begin(), rankedList.end(), 
         nor_utils::comparePair<2, int, float, greater<float> >() );
}
