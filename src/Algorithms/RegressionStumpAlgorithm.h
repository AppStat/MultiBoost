...
/**
 * \file RegressionStumpAlgorithm.h The Decision Stump-based algorithms.
 */

#ifndef __REGRESSION_STUMP_ALGORITHM_H
#define __REGRESSION_STUMP_ALGORITHM_H
...
namespace MultiBoost {
     ...
     */
    template <typename T>
        class RegressionStumpAlgorithm
    {
    public:
        ...
        RegressionStumpAlgorithm( int numClasses )
        {
        ...
        FeatureReal findSingleThreshold(const vpIterator& dataBegin,
                                        ...
                                        vector<sRates>* pMu = NULL, vector<AlphaReal>* pV = NULL, vector<AlphaReal>* pV2 = NULL);
    ...
    template <typename T> 
        void RegressionStumpAlgorithm<T>::initSearchLoop(InputData* pData)
    {
    ...
    template <typename T> 
        FeatureReal RegressionStumpAlgorithm<T>::findSingleThreshold(const vpIterator& dataBegin,
                                                                     const vpIterator& dataEnd,
                                                                     InputData* pData,
                                                                     AlphaReal halfTheta,
                                                                     vector<sRates>* pMu, vector<AlphaReal>* pV, vector<AlphaReal>* pV2)
    { 
        // initialize halfEdges to the constant classifier's half edges 
        initSearchLoop(pData);

        // findSingleThreshold after initialization
        return findSingleThresholdWithInit(dataBegin,dataEnd,pData,halfTheta,pMu,pV,pV2);

    } // end of findSingleThreshold

    template <typename T> 
        FeatureReal RegressionStumpAlgorithm<T>::findSingleThresholdWithInit
        (const vpIterator& dataBegin,const vpIterator& dataEnd,
         InputData* pData, AlphaReal halfTheta, vector<sRates>* pMu, vector<AlphaReal>* pV, vector<AlphaReal>* pV2 )
    { 
        ...
        {
            float threshold = static_cast<FeatureReal>( bestPreviousSplitPos->second + 
    ...
    template <typename T> 
        void RegressionStumpAlgorithm<T>::findMultiThresholds(const vpIterator& dataBegin,
                                                              const vpIterator& dataEnd,
                                                              InputData* pData, vector<FeatureReal>& thresholds,
    ...
    template <typename T> 
        void RegressionStumpAlgorithm<T>::findMultiThresholdsWithInit
        (const vpIterator& dataBegin, const vpIterator& dataEnd,
         InputData* pData, vector<FeatureReal>& thresholds, 
         vector<sRates>* pMu, vector<AlphaReal>* pV)
    { 
