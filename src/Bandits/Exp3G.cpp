...
//----------------------------------------------------------------
    void Exp3G::initLearningOptions(const nor_utils::Args& args) 
    {
        if ( args.hasArgument( "gamma" ) ){
            _gamma = args.getValue<AlphaReal>("gamma", 0 );
        } 
...
//----------------------------------------------------------------
    ...
    void Exp3G::initialize( vector< AlphaReal >& vals )
    {
...
//----------------------------------------------------------------
//----------------------------------------------------------------
    void Exp3G::receiveReward( int armNum, AlphaReal reward )
    {
        _T[ armNum ]++;
        ...
        for( int i=0; i < _numOfArms; i++ ) 
        {
            _w[i] += ( ( ( (AlphaReal)_sideInformation[i][prevArm] ) / ( (AlphaReal)_sideInformation[armNum][prevArm] ) ) * ( _eta * reward ) );
        }
