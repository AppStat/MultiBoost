...
//--------------------------------------------------------------------
    ...
    void ClassHierarchy::collectCategories( InnerNode* currNode, NameMap& namemap, map<int,int>& idxtocat, map<int,int>& cattoidx, map<int,InnerNode*>& cattonode ) {
        ...
        idxtocat.insert( pair<int,int>( curridx, currNode->getCategory() ) );
        cattoidx.insert( pair<int,int>( currNode->getCategory(), curridx ) );
        cattonode.insert( pair<int,InnerNode*>( currNode->getCategory(), currNode ) );
    ...
    void ClassHierarchy::addHierarchPath( vector<int>& hierarchyPath ){
        ...
        for( vector<int>::iterator it = hierarchyPath.begin(); it != hierarchyPath.end(); it++ ) {
            if ( currNode->hasChildWithThisCategory( *it ) ) {
                ...
                _mapIdxToCategory.insert( pair<int,int>( _numOfCategories, *it ) );
                _mapCategoryToIdx.insert( pair<int,int>( *it, _numOfCategories ) );
                _numOfCategories++;

                currNode = currNode->getChild( *it );
                _mapCategoryToNode.insert( pair<int,InnerNode*>( *it, currNode ) );
