...
//REGISTER_LEARNER_NAME(SingleStump, SingleSparseStump)
    ...
    AlphaReal SingleSparseStump::run()
    {
        ...
        vector<FeatureReal> tmpThresholds;
        _threshold.resize(2);
        ...
        SparseStumpAlgorithm<FeatureReal> sAlgo(numClasses);
...
// ------------------------------------------------------------------------------
    ...
    AlphaReal SingleSparseStump::phi(FeatureReal val) const
    {
        if ( val < _threshold[1] )
            return -1;
...
// ------------------------------------------------------------------------------
    ...
    AlphaReal SingleSparseStump::phi(InputData* pData,int pointIdx) const
    {
