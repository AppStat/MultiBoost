...
//REGISTER_LEARNER_NAME(SingleStump, SingleRegressionStumpLearner)
    ...
    AlphaReal SingleRegressionStumpLearner::run()
    {
        ...
        vector<AlphaReal> tmpV2(numClasses); // The class-wise votes/abstentions
        ...
        RegressionStumpAlgorithm<FeatureReal> sAlgo(numClasses);
...
// ------------------------------------------------------------------------------
    ...
    AlphaReal SingleRegressionStumpLearner::run( int colIdx )
    {
        const int numClasses = _pTrainingData->getNumClasses();
        const int numColumns = _pTrainingData->getNumAttributes();
...
// ------------------------------------------------------------------------------
    ...
    AlphaReal SingleRegressionStumpLearner::phi(FeatureReal val, int /*classIdx*/) const
    {
        if (val > _threshold)
            return +1;
        else
...
// ------------------------------------------------------------------------------
    ...
    AlphaReal SingleRegressionStumpLearner::phi(InputData* pData,int pointIdx) const
    {
