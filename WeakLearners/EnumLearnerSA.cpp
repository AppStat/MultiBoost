...
namespace MultiBoost {
		...
		float EnumLearnerSA::run()
	{
		...
		float EnumLearnerSA::run( int colIdx )
	{
		const int numClasses = _pTrainingData->getNumClasses();
		const int numColumns = _pTrainingData->getNumAttributes();
		const int numExamples = _pTrainingData->getNumExamples();
	...
	float EnumLearnerSA::phi(float val, int /*classIdx*/) const
	{
	...
	void EnumLearnerSA::save(ofstream& outputStream, int numTabs)
	{
	...
	void EnumLearnerSA::load(nor_utils::StreamTokenizer& st)
	{
		// Calling the super-class method
	...
	void EnumLearnerSA::subCopyState(BaseLearner *pBaseLearner)
	{
		FeaturewiseLearner::subCopyState(pBaseLearner);

		EnumLearnerSA* pEnumLearnerSA =
			dynamic_cast<EnumLearnerSA*>(pBaseLearner);

		pEnumLearnerSA->_u = _u;
