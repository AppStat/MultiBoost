...
namespace MultiBoost {
	...
	//REGISTER_LEARNER_NAME(SingleStump, EnumLearner)
	REGISTER_LEARNER(EnumLearner)

		// ------------------------------------------------------------------------------

		float EnumLearner::run()
	{
	...
	float EnumLearner::phi(float val, int /*classIdx*/) const
	{
	...
	void EnumLearner::save(ofstream& outputStream, int numTabs)
	{
	...
	void EnumLearner::load(nor_utils::StreamTokenizer& st)
	{
		// Calling the super-class method
	...
	void EnumLearner::subCopyState(BaseLearner *pBaseLearner)
	{
		FeaturewiseLearner::subCopyState(pBaseLearner);

		EnumLearner* pEnumLearner =
			dynamic_cast<EnumLearner*>(pBaseLearner);
